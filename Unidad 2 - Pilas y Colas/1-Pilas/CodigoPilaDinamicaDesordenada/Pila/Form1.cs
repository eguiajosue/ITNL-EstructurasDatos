...
namespace Pila
{
    ...
    {
        PilaDinamicaDesordenada<CentroCultural> listaCentrosCulturales = new PilaDinamicaDesordenada<CentroCultural>();
        public Form1()
        {
            InitializeComponent();
        }
        ...
        private void btnAgregar_Click(object sender, EventArgs e)
        {
            try {
                ...
                if (result == DialogResult.Yes) {
                    ...
                    listaCentrosCulturales.Push(centroCultural);
        ...
        private void btnGenerarAleatorio_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < 10; i++) {
                try {
                    CentroCultural randomCentroCultural = new CentroCultural();

                    randomCentroCultural.CodigoPostal = random.Next(intCodigoPostalMinimo, intCodigoPostalMaximo);
                    randomCentroCultural.Presupuesto = random.Next();
                    randomCentroCultural.Posicionamiento = arrayPosiciones[random.Next(arrayPosiciones.Length)];
                    randomCentroCultural.FechaConstruccion = new DateTime(random.Next(1950, 2021), random.Next(1, 13), random.Next(1, 31));
                    randomCentroCultural.Mantenimiento = arrayMantenimiento[random.Next(arrayMantenimiento.Length)];
                    randomCentroCultural.Sector = arrayTipoSector[random.Next(arrayTipoSector.Length)];
                    randomCentroCultural.RutaImagen = arrayImagenes[random.Next(arrayImagenes.Length)];

                    listaCentrosCulturales.Push(randomCentroCultural);
                    CargarDatos();
                    LimpiarDatos();
            ...
            try
            {
                if (dtgCentrosCulturales.SelectedRows.Count == 1)
                {
                    ...
                    if (result == DialogResult.Yes)
                    {
                        ...
                        listaCentrosCulturales.Pop(centroCultural);
                        CargarDatos();
                        LimpiarDatos();
                    }
                }
